...
if ORDER_MODE == "plan_after":
    st.info(
        ...
        icon="🧭")
    try:
        st.page_link("views/5_📝_Order_Sheet.py",
                     label="📝 Go to the Order Sheet round",
                     use_container_width=True)
    except Exception:
        # AppTest runtimes cannot resolve page links (no navigation
        # registry); the pointer must survive there too.
        st.markdown("📝 **Order Sheet** → the ordering round")
    st.divider()
            ...
            if view_mode == "Table (Desktop)":
                ...
                for item in items:
                    ...
                    if qty > 0:
                        ...
                        # Vendor override (Phase C): the manager can
                        # change from whom, not just how much. An
                        # overridden line is recorded as the manager's call.
                        order_items[-1] = vendor_override.render(
                            db, {**order_items[-1], 'name': item['item']},
                            key=f"ovr_table_{item['item']}")
