...
@pytest.mark.parametrize("doc_name", ["MOCK_RESTAURANT.md", "TEST_DRIVE.md"])
def test_documented_commands_point_at_files_that_exist(doc_name):
    ...
    import re
    doc = (REPO / "docs" / doc_name).read_text()
    targets = re.findall(r"(?:streamlit run|python)\s+(\S+\.py)", doc)
    assert targets, f"{doc_name} should show how to run it"
    for target in targets:
        assert (REPO / target).exists(), \
            f"{doc_name} references missing {target}"
